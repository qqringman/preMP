...
class ManifestConverter:
    """簡化版 Manifest 轉換器 - 修正版本，支援 dest-branch、upstream 和 upgrade 識別"""
    ...
    def convert_manifest(self, input_file: str, conversion_type: str, output_file: str = None) -> bool:
        """
        轉換 manifest 檔案
        
        Args:
            input_file: 輸入的 manifest 檔案路徑
        ...
        try:
            ...
            # 生成輸出檔案名稱
            if not output_file:
                output_file = self._generate_output_filename(input_file, conversion_type)
            ...
            # 顯示使用說明
            self._show_usage_instructions(conversion_type, output_file)
    ...
    def _generate_output_filename(self, input_file: str, conversion_type: str) -> str:
        """生成輸出檔案名稱"""
    ...
    def _show_usage_instructions(self, conversion_type: str, output_file: str):
        """顯示使用說明"""
        print("\n" + "="*60)
        print("🎉 轉換完成！以下是使用轉換後 manifest 的說明：")
        print("="*60)
...
def interactive_mode():
    ...
    print("="*60)
    print("🔧 Manifest 轉換工具 - 互動模式")
    print("="*60)
    
    # 選擇輸入檔案
    while True:
        input_file = input("\n請輸入 manifest 檔案路徑: ").strip()
    ...
    success = converter.convert_manifest(input_file, conversion_type, output_file)
...
def main():
    """主函數"""
    parser = argparse.ArgumentParser(description='Manifest 轉換工具')
    parser.add_argument('input_file', nargs='?', help='輸入的 manifest 檔案')
    ...
    converter = ManifestConverter()
    success = converter.convert_manifest(args.input_file, args.type, args.output)
